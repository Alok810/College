...
return (
    <RequireVerification>
      {/* 🟢 1. Completely stopped outer scrolling and fixed crop clipping */}
       <div className="flex flex-col flex-1 items-center h-[calc(100dvh-60px)] sm:h-[calc(100vh-80px)] w-full max-w-[100vw] overflow-hidden -mt-4 sm:pt-4 pb-20 sm:pb-4">
        <div className="flex flex-col flex-1 w-[94%] sm:w-full max-w-6xl mx-auto h-full min-h-0 gap-3 sm:gap-4 relative">
          ...
          {/* 🟢 2. Normal top margin (removed negative margins) */}
          <div className="flex-shrink-0 w-full md:w-max md:mx-auto overflow-hidden bg-white p-1.5 rounded-xl shadow-sm border border-slate-200 z-10">
            <div className="flex gap-1 justify-center sm:justify-start overflow-x-auto custom-scrollbar">
              {tabs.map((tab, index) => (
                <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={`relative px-3 sm:px-5 py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all whitespace-nowrap flex items-center gap-1.5 ${activeTab === tab.id ? 'bg-[#4F46E5] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}>
          ...
          {/* 🟢 3. White card is constrained. overflow-hidden forces scrolling inside it. */}
          <div className="flex-1 bg-white shadow-sm rounded-[1.5rem] p-4 sm:p-6 border border-slate-100 flex flex-col min-h-0 overflow-hidden">
            ...
            {/* 🟢 DYNAMIC TOP HEADER (ALWAYS STICKY) */}
            ...
            {/* 🟢 TABS CONTENT WRAPPER (Inner Scrolling handled here) */}
            <div className={`flex-1 flex flex-col min-h-0 ${activeTab === 'overview' ? 'overflow-y-auto custom-scrollbar pr-2 pb-2' : 'overflow-hidden'}`}>
                ...
                <div className="flex flex-col w-full animate-in fade-in duration-300">
                  ...
                  {clubs.length > 0 && (
                      <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">{clubs.slice(0, 3).map(club => <ClubCard key={club._id} club={club} authData={authData} onJoinRequest={handleRequestJoin} onLeaveClub={handleLeaveOrRemove} onViewDetails={(id) => { handleTabChange('clubs'); setViewingClubId(id); }}/>)}</div>
                      </>
                  )}
                </div>
              )}
              ...
              {/* --- 🟢 TAB 2: COUNCIL --- */}
              {activeTab === 'council' && (
                <div className="flex flex-col flex-1 w-full animate-in fade-in duration-300 min-h-0">
              ...
              {activeTab === 'clubs' && (
                <div className="flex flex-col w-full animate-in fade-in duration-300 h-full min-h-0">
                  
                  {/* GRID VIEW */}
                  {!viewingClubId && (
                     <div className="flex flex-col h-full min-h-0 w-full">
                        {/* 🟢 4. The sub-header is now sticky inside the scroll area! */}
                        ...
                        {/* Grid is independently scrollable */}
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
                            ...
                            {clubs.length === 0 && (<div className="text-center py-20 bg-slate-50 rounded-2xl border border-dashed border-slate-200 mt-4"><Tent className="w-12 h-12 text-slate-300 mx-auto mb-3" /><h3 className="text-lg font-black text-slate-800 mb-1">No Clubs Found</h3><p className="text-sm text-slate-500 font-medium">No official clubs have been created on the platform yet.</p></div>)}
                        </div>
                     </div>
                  )}
                  ...
                  {viewingClubId && activeDetailClub && (
                     <div className="flex flex-col w-full h-full flex-shrink-0 animate-in slide-in-from-right-8 duration-300 relative min-h-0">
                        ...
                        <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0 overflow-hidden">
                            ...
                            <div className="w-full lg:w-2/3 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2 pb-2">
                            ...
                            <div className="w-full lg:w-1/3 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2 pb-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            {activeClubSubClubs.map(sub => (
                                                ...
                                                >
                                                    ...
                                                    <ChevronRight size={16} className="text-slate-300 group-hover:text-indigo-500 flex-shrink-0" />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 pt-4 border-t border-slate-100 flex-shrink-0 pb-2 gap-4">
                            <button 
              ...
              {activeTab === 'manage' && myClub && (
                <div className="flex flex-col h-full w-full animate-in fade-in duration-300 min-h-0">
                  
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 mb-4 p-5 bg-indigo-50/50 border border-indigo-100 rounded-2xl flex-shrink-0">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0 overflow-hidden pb-1">
                     
                     <div className="lg:col-span-1 flex flex-col h-full gap-6 overflow-y-auto custom-scrollbar pr-2 pb-2">
                        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex-shrink-0">
                        ...
                        <div className="bg-white border border-indigo-200 rounded-2xl p-5 shadow-sm flex-shrink-0 flex flex-col max-h-[350px]">
                     ...
                     <div className="lg:col-span-2 flex flex-col h-full gap-6 overflow-y-auto custom-scrollbar pr-2 pb-2">
                        {rosterFilter === 'pending' && (
                           <div className="bg-white border border-amber-200 rounded-2xl p-5 shadow-sm flex-shrink-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        {(rosterFilter === 'all' || rosterFilter === 'core') && (
                           <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col flex-1 min-h-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
                              <div className="flex items-center justify-between mb-4 flex-shrink-0">
                              ...
                              <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden flex-1 flex flex-col min-h-0">
