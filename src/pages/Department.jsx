...
export default function Department() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      // ✅ FIX: Use Fallback so it finds students even if abbreviation is missing
    ...
    const fetchBlueprint = async () => {
      if (academicBatch && academicSemester && activeDept) {
        try {
          ...
          // ✅ FIX: Use Fallback to find old blueprint documents
          const targetBranch = activeDept.abbreviation || activeDept.name;
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-[100vw] overflow-x-hidden pt-1 sm:pt-2 pb-24 sm:pb-6 relative">
      
      <div className="w-[94%] sm:w-full max-w-6xl mx-auto flex flex-col gap-2 sm:gap-3">
        ...
        <div className="w-full overflow-hidden bg-white p-1.5 rounded-xl shadow-sm border border-slate-200">
          <div className="flex gap-1 justify-start sm:justify-center overflow-x-auto custom-scrollbar snap-x snap-mandatory pb-1 sm:pb-0">
            <button onClick={() => setActiveTab('overview')} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'overview' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Overview</button>
            <button onClick={() => setActiveTab('hod')} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-1.5 ${activeTab === 'hod' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}><Shield size={14}/> HOD</button>
            <button onClick={() => setActiveTab('academics')} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'academics' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Academics</button>
            <button onClick={() => {setActiveTab('faculty'); setSearchQuery("");}} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'faculty' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Faculty</button>
            <button onClick={() => {setActiveTab('students'); setSearchQuery("");}} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'students' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Students</button>
            <button onClick={() => {setActiveTab('alumni'); setSearchQuery("");}} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'alumni' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Alumni</button>
            <button onClick={() => setActiveTab('research')} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'research' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Research</button>
            <button onClick={() => setActiveTab('notices')} className={`snap-start px-4 sm:px-6 py-2.5 rounded-lg font-bold text-xs transition-all whitespace-nowrap flex-shrink-0 ${activeTab === 'notices' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}>Notice Board</button>
          </div>
        </div>

        {/* 🟢 MAIN CONTENT CARD */}
        <div className="bg-white shadow-sm rounded-[1.5rem] p-4 sm:p-6 border border-slate-100 w-full flex flex-col min-h-[60vh]">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4 mb-5 gap-4">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <select 
                className="w-full md:w-64 p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-indigo-400 font-bold text-sm text-slate-800 shadow-sm transition-colors cursor-pointer"
          ...
          {activeTab === 'academics' && activeDept && (
            <div className="w-full flex flex-col gap-4 animate-in fade-in duration-300">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-100 pb-3 mb-2 gap-3">
          ...
          {activeTab === 'faculty' && activeDept && (
            <FacultyDirectory 
              activeDept={activeDept} 
              canEdit={false} // 🟢 Set to false since HOD tab now handles assignments
              onRefresh={fetchDepartments} 
            />
          )}
          ...
          {activeTab === 'students' && activeDept && (
            <div className="w-full flex flex-col gap-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2 border-b border-slate-100 pb-3">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Student Roster</h3>
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                  <select 
                    className="p-2.5 border border-slate-200 rounded-lg text-sm outline-none bg-slate-50 font-bold focus:border-indigo-400" 
                  ...
                  <div className="relative w-full sm:w-64">
                    ...
                    <input type="text" placeholder="Search by name or reg no..." className="w-full p-2.5 pl-9 border border-slate-200 rounded-lg text-sm outline-none bg-slate-50 focus:bg-white font-bold text-slate-700" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  </div>
                </div>
              </div>
              ...
              {/* 🟢 CSV DOWNLOAD BUTTON */}
              {deptStudents.length > 0 && !studentsLoading && (
                <div className="mt-2 flex justify-end border-t border-slate-100 pt-5">
                  <button 
                    onClick={handleDownloadStudentList}
                    className="flex items-center gap-2 px-6 py-3 bg-slate-900 hover:bg-black text-white font-extrabold text-sm rounded-xl shadow-md transition-all active:scale-95"
                  >
                    <Download size={16} /> Download Student List (CSV)
                  </button>
                </div>
              )}
            </div>
          ...
          {activeTab === 'notices' && activeDept && (
            <div className="w-full flex flex-col gap-4 animate-in fade-in duration-300">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-amber-50 border border-amber-200 p-4 rounded-2xl shadow-sm">
