...
const StudentClassView = memo(({ results, users, currentPage, totalPages, setCurrentPage }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      <div className="bg-white shadow-sm rounded-[1.5rem] p-4 sm:p-6 border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 w-full pb-0">
      ...
      {totalPages > 1 && (
        <div className="flex justify-between sm:justify-center items-center gap-2 sm:gap-4 pt-2 sm:pt-4 pb-0 mt-0">
          <button 
...
const AdminDashboard = memo(({ results, users, departments, handleUpload, handleDelete, fetchAdminData, displayMessage, currentPage, totalPages, setCurrentPage }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="bg-white shadow-sm rounded-[1.5rem] p-4 sm:p-6 border border-slate-100 w-full flex flex-col">
...
const Result = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    try {
      if (isOfficial) {
        ...
        // ✅ ADDED Department Fetch for dynamic branches
        try {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-[100vw] overflow-x-hidden pt-1 sm:pt-2 pb-0">
      ...
      <div className="w-[94%] sm:w-full max-w-6xl mx-auto flex flex-col gap-2 sm:gap-3">
        {isOfficial ? (
          <AdminDashboard 
            results={results} 
            users={users} 
            departments={departments} // ✅ PASSED DEPARTMENTS DOWN
